		...
		static function init_static_blocks() {
			if (function_exists('register_post_type')) {
				register_post_type('static_block',
					array(
						'labels' => array(
							'name' 					=>	esc_html_x('Static Content Blocks', 'post type general name', 'Alita-extensions'),
							'singular_name' 		=>	esc_html_x('Static Block', 'post type singular name', 'Alita-extensions'),
							'add_new' 				=>	esc_html_x('Add New', 'block', 'Alita-extensions'),
							'add_new_item' 			=>	esc_html__('Add New Block', 'Alita-extensions'),
							'edit_item' 			=>	esc_html__('Edit Block', 'Alita-extensions'),
							'new_item' 				=>	esc_html__('New Block', 'Alita-extensions'),
							'all_items' 			=>	esc_html__('Static Blocks', 'Alita-extensions'),
							'view_item' 			=>	esc_html__('View Block', 'Alita-extensions'),
							'search_items' 			=>	esc_html__('Search', 'Alita-extensions'),
							'not_found' 			=>	esc_html__('No blocks found', 'Alita-extensions'),
							'not_found_in_trash' 	=>	esc_html__('No blocks found in Trash', 'Alita-extensions'), 
							'parent_item_colon' 	=> '',
...
$meta_box_static_blocks = array(
	...
	'title' 	=>  esc_html__('Content Options', 'Alita-extensions'),
	'page' 		=> 'static_block',
	'context' 	=> 'side',
	'priority' 	=> 'default',
	'fields' 	=> array(
    	array(
    	   'name' 		=> esc_html__('Content Filters', 'Alita-extensions'),
    	   'desc' 		=> esc_html__('Apply all WP content filters? This will include plugin added filters.', 'Alita-extensions'),
    	   'id' 		=> 'content_filters',
    	   'type' 		=> 'radio',
    	   'std' 		=> '',
    	   'options'	=> array(
    	   		'default'	=> esc_html__('Defaults (recommended)', 'Alita-extensions'),
    	   		'all' 		=> esc_html__('All Content Filters', 'Alita-extensions')
    	   	)
    	),
    	array(
    	   'name' 		=> esc_html__('Auto Paragraphs', 'Alita-extensions'),
    	   'desc' 		=> esc_html__('Add &lt;p&gt; and &lt;br&gt; tags automatically. (disabling may fix layout issues)', 'Alita-extensions'),
    	   'id' 		=> 'wpautop',
    	   'type' 		=> 'radio',
    	   'std' 		=> '',
    	   'options' 	=> array(
    	   		'on' 	=> esc_html__('On', 'Alita-extensions'),
    	   		'off' 	=> esc_html__('Off', 'Alita-extensions')
    	   	)
    	)
   	)
);
...
function Alita_add_box_static_blocks() {
	global $meta_box_static_blocks;
	
	add_meta_box(
		$meta_box_static_blocks['id'], 
		$meta_box_static_blocks['title'], 
		'Alita_show_box_static_blocks', 
...
add_action('admin_menu', 'Alita_add_box_static_blocks');
...
function Alita_show_box_static_blocks() {
	global $meta_box_static_blocks, $post;
 	
	// Use nonce for verification
	echo '<input type="hidden" name="Alita_meta_box_nonce" value="', wp_create_nonce( plugin_basename(__FILE__) ), '" />';
...
add_action('save_post', 'Alita_save_data_static_blocks');
...
function Alita_save_data_static_blocks($post_id) {
	global $meta_box_static_blocks;
 
	// verify nonce
	if ( !isset($_POST['Alita_meta_box_nonce']) || !wp_verify_nonce($_POST['Alita_meta_box_nonce'], plugin_basename(__FILE__))) {
		return $post_id;
	}
